...
public class ConfigLoader {
    ...
    private static final List<String> DEFAULT_SQL_PAYLOADS = Arrays.asList(
        "'",
        "1' OR '1'='1",
        "' OR 1=1--",
        "' UNION SELECT NULL--",
    ...
    private static final List<String> DEFAULT_XSS_PAYLOADS = Arrays.asList(
    ...
    private static final Map<String, String> DEFAULT_SENSITIVE_PATTERNS = Map.ofEntries(
    ...
    private static ScannerConfig validateAndFillDefaults(ScannerConfig config) {
        List<String> sqlPayloads = (config.sqlPayloads() == null || config.sqlPayloads().isEmpty()) 
            ? DEFAULT_SQL_PAYLOADS : config.sqlPayloads();
            
        List<String> xssPayloads = (config.xssPayloads() == null || config.xssPayloads().isEmpty()) 
            ? DEFAULT_XSS_PAYLOADS : config.xssPayloads();
            
        Map<String, String> sensitivePatterns = (config.sensitivePatterns() == null || config.sensitivePatterns().isEmpty()) 
            ? new HashMap<>(DEFAULT_SENSITIVE_PATTERNS) : config.sensitivePatterns();
            
        ScanSettings scanSettings = (config.scanSettings() == null) 
            ? ScanSettings.defaultSettings() : config.scanSettings();
